...
class DeletionAwareImeTextEditingController
    extends ImeAttributedTextEditingController {
  ...
  AttributedText get text =>
      _hasInvisiblePrefix ? _text.copyText(_invisiblePrefix.length) : _text;
  ...
  set selection(TextSelection newValue) {
    print(
        "Setting selection to: $newValue, has prefix? $_hasInvisiblePrefix, is valid? ${newValue.isValid}");
    ...
    _hasInvisiblePrefix && newValue.isValid
        ...
        : super.selection = newValue.isValid
            ? newValue
  ...
  TextRange get composingRegion =>
      _hasInvisiblePrefix && _composingRegion.isValid
          ? TextRange(
  ...
  String extractLeadingToken() {
    ...
    print(" - token: $token");
    update(
      text: _text.removeRegion(
          startOffset: _invisiblePrefix.length, endOffset: spaceIndex + 1),
  ...
  void delete(
    ...
    final adjustedSelection = newSelection != null && newSelection.isValid
        ? TextSelection(
            ...
            extentOffset:
                max(newSelection.extentOffset, _invisiblePrefix.length),
          )
        : null;

    final adjustedComposingRegion =
        newComposingRegion != null && newComposingRegion.isValid
            ? TextRange(
